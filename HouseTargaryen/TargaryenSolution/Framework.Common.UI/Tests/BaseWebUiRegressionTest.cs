namespace Framework.Common.UI.Tests
{
    ...
    /// <summary>
    /// The base class for WLN regression tests for QRT
    /// </summary>
    [TestClass]
    public class BaseWebUiRegressionTest : BaseContextualTest<UiTestExecutionContext>
    {
        private ScreenshotTaker screenshotTaker;

        private Task qualityTestCaseObserverTask;

        private bool isTestRunComplete;

        /// <summary>
        /// Initializes a new instance of the <see cref="BaseWebUiRegressionTest"/> class. 
        /// </summary>
        public BaseWebUiRegressionTest()
        {
            ...
            get
            {
                return
                    this.screenshotTaker =
                        this.screenshotTaker ?? new ScreenshotTaker(this.TestContext, this.QualityTestCase, Thread.CurrentThread.ManagedThreadId);
            }
        ...
        /// </summary>
        [TestCleanup]
        public override sealed void CleanupTestCase()
        {
        ...
        protected void DisplayPasswordInfo()
        {
            foreach (IUserInfo userInfo in CredentialPool.GetAllRegisteredUsers())
            {
                ...
                if (userDbInfo != null)
                {
                    ...
                    BaseWebUiRegressionTest.PrintPasswordInfo(userDbInfo.PasswordInfo);
                }
                else
                {
        ...
        {         
            ...
            if (this.Settings.GetValue(EnvironmentConstants.IsFedRamp).ToLower() == "yes")
            {
                this.Settings.AppendValues(
                    EnvironmentConstants.FeatureAccessControlsOn,
                    SettingUpdateOption.Append,
                    FeatureAccessControl.FedRampWestlawUS);
            }
        }
        ...
        {
            // TODO: revise the logic for taking screenshot after all failed checks
            // take a screenshot if we hit an exception - it does not take a screenshot if a check failed
            if (this.TestContext.CurrentTestOutcome == UnitTestOutcome.Failed
                || this.QualityTestCase.QualityChecks.Any(
                    c => c.Outcome == Outcome.Failed || c.Outcome == Outcome.Inconclusive))
            {
        ...
        /// <summary>
        /// Background task, that performs monitoring QualityChecks status
        /// and takes screenshot if any check has been failed
        /// </summary>
        private void ObserveQualityChecks()
        {
            var checkedList = new List<string>();
            QualityCheck failedCheck = null;
            while (!this.isTestRunComplete)
            {
                SafeMethodExecutor.Execute(
                    () =>
                        {
                            failedCheck =
                                this.QualityTestCase?.QualityChecks.FirstOrDefault(
                                    x => x.Outcome == Outcome.Failed && !checkedList.Contains(x.Name));

                            if (failedCheck != null)
                            {
                                this.ScreenshotTaker.TakeScreenshot();
                                checkedList.Add(failedCheck.Name);
                            }
                        });
            }
        }
        ...
        private void PerformUiPostconditions()
        {
            ...
            if (this.UiExecutionSettings.IsFlagDependencyMet(UiFlagDependencyMap.AllowScreenshotOnFailedQualityCheck))
            {
                this.DisposeQualityObserverTask();
            }
        ...
        /// <summary>
        /// Starts the QualityObserver task
        /// </summary>
        private void StartQualityObserverTask()
        {
            this.screenshotTaker = new ScreenshotTaker(this.TestContext, this.QualityTestCase, Thread.CurrentThread.ManagedThreadId);
            this.qualityTestCaseObserverTask = Task.Factory.StartNew(this.ObserveQualityChecks);
        }

        /// <summary>
        /// Disposes the run of QualityObserver task
        /// </summary>
        private void DisposeQualityObserverTask()
        {
            if (this.qualityTestCaseObserverTask.Status == TaskStatus.RanToCompletion)
            {
                this.qualityTestCaseObserverTask.Dispose();
            }
        }
    }
}